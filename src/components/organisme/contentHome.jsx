...
function ContentHome({ load, epsd, hiddenContent, qr }) {
    ...
    return (
        <>
            {hiddenContent ? <></> : (
                <>
                    <div className="mb-4 mt-8 flex items-center justify-end font-bold lg:text-xl">
                        <h3 className="text-white lg:border-l-2 border-indigo-400 lg:px- lg:border-none"><Link to='/newEpisode'>New Episode</Link></h3>
                        <div className="text-indigo-400"><FcNext /></div>
                    </div>
            ...
            {load ? <Load /> : <Card data={data} load={load} type={`grid grid-cols-2 gap-8 lg:grid-cols-5 lg:gap-12 ${hiddenContent ? 'mt-12' : ''}`} epsd='hidden' />}
        </>
    );
}
